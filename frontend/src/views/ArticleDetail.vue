<template>
  <div class="article-detail" v-if="article">
    ...
    <div class="meta">
      ...
      <router-link v-if="auth.isAdmin" :to="'/write/' + article.id" class="edit-link"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg> 编辑</router-link><button v-if="auth.isAdmin" @click="deleteArticle" class="del-link"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg> 删除</button>
    </div>
...
const displayContent = computed(() => {
  ...
  text = text.replace(/^#\s+.+/, (m: string) => {
    const h = m.replace(/^#\s+/, '').trim()
    return h === article.value!.title ? '' : m
  })
  return text
})
...
async function fetchArticle() {
  ...
  article.value = res.data; loading.value = false
}
...
async function postComment() {
  ...
  commentText.value = ''; fetchComments()
}
async function deleteComment(id: number) { await api.delete('/api/comments/' + id); fetchComments() }
...
<style scoped>
.article-detail { max-width: 800px; margin: 0 auto; padding: 2rem 1rem; }
...
.content-wrap :deep(.md-editor) { height: auto !important; border: none; }
.content-wrap :deep(.md-editor-preview-wrapper) { padding: 0; }

.cut-line { display: flex; align-items: center; gap: 0.8rem; margin-bottom: 1.5rem; }
